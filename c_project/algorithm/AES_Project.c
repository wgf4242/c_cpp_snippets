...
int StringToArray(const unsigned char *String, unsigned char (*Array)[4])
{
	...
	for (int i = 0; i < 16; i++)
	{
        Array[i & 0x03][i >> 2] = String[i]; // & 0b11 相当于 % 4, 分到4x4的数组里
    }
    /*
     * 01234567ABCDEFGH => 4个一组 分4组
     * [048C,159D,26AE,37BF]
     */
...
int CalculateExtendKeyArray(const unsigned char(*PasswordArray)[4], unsigned char(*ExtendKeyArray)[44])
{
	...
	for (int i = 0; i < 16; i++)
	{
        // 密钥数长度44, 我的示例中PasswordArray只有16位, /4 每组只填充了前4位
		ExtendKeyArray[i & 0x03][i >> 2] = PasswordArray[i & 0x03][i >> 2]; // 4个1组取第i % 4行第 i // 4列
	}
