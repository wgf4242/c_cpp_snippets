...
void print_usage(const char* program_name) {
    LOG_PRINTF("用法: %s [选项]\n", program_name);
    LOG_PRINTF("选项:\n");
    LOG_PRINTF("  -i <IP>     服务器 IP 地址 (默认: %s)\n", DEFAULT_IP);
    LOG_PRINTF("  -p <端口>   服务器端口 (默认: %s)\n", DEFAULT_PORT);
    LOG_PRINTF("  -h          显示此帮助信息\n");
    LOG_PRINTF("\n示例:\n");
...
SOCKET tcp_connect(const char* host, const char* port) {
    ...
    while (1) {
        ...
        if (ret != 0) {
            LOG_PRINTF("getaddrinfo 失败: %d\n", ret);
            LOG_PRINTF("将在 5 秒后重试...\n");
            Sleep(5000);
            continue;
        }
